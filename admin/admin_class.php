...
Class Action {
	...
	function createproduct()
	{
		...
		$qry = $this->db->query("INSERT INTO products(p_name, product_id, cat_id, product_price, product_discount, product_total, product_desc,product_feat, product_selling, product_sale, product_arrival) VALUES ('$productname', 'TEST123', '$categoryname', '$price', '$disc1', '$discount_price', '$description', '$featured', '$best_sale', '$on_sale', '$new_arrival')");
		...
		$qry = $this->db->query("UPDATE products SET product_id='$productid' WHERE product_id = 'TEST123'");
...
function reviewstatus()
	...
	function statetax(){
		extract($_POST);
		$result=$this->db->query("INSERT INTO sgst(state_id, taxpercent, status) VALUES ('$state','$percentage',1)");
		return $result;
	}

	function taxstatus(){
		$id=$_POST['id'];
		$qry = $this->db->query("SELECT * FROM sgst where id='$id'");
		$res = $qry->fetch_array()[3];
		if($res==1){
			$qry2 = $this->db->query("UPDATE sgst SET status=2 WHERE id='$id'");
		}
		else if($res==2){
			$qry2 = $this->db->query("UPDATE sgst SET status=1 WHERE id='$id'");
		}
		return $qry2;
	}
	...
	function gettaxper(){
		$id=$_POST['id'];
		$qry = $this->db->query("SELECT * FROM sgst where state_id='$id'");
		$res = $qry->fetch_array();
		return $res['taxpercent'];
	}
	
	function statetaxupdate(){
		extract($_POST);
		$result=$this->db->query("UPDATE sgst SET taxpercent='$editpercentage' WHERE state_id = '$state'");
		echo $this->db->error;
		return $result;
	}
